/*  BEFORE USING THIS CODE MAKE SURE YOU HAVE ESP-32 AI Thinker cam
 *  
 *  INTERNAL TEMPERATURE SENSOR DEFINING BELOW
 *  13 PIN MOTION PIR SENSOR
 *  2 PIN PHOTORESISTOR 
 */
#ifdef __cplusplus
  extern "C" {
...
const String allowed_chat_id = "405759266";
...
String info(String chat_id) {
  ...
  String infoFlash = String("Flash before photo: " + String(flash ? "Yes" : "No"));
  ...
  String infoLightLevel = String("Light level: " + String(lightLevel) + "\nmax: " + String(maxlight) + "\nmin: " + String(minlight));
...
void loop() {
  ...
  if (currentMillis - previousMillisTrig > periodTrig) {
    ...
    if (armed && digitalRead(TRIGGER_PIN) == HIGH) {
      take_send_photo(allowed_chat_id);
      lightLevel = analogRead(PHOTO_RESISTOR_PIN);
      flash = lightLevel <= 2000 ? true : false; 
      maxlight = lightLevel > maxlight ? lightLevel : maxlight;
      minlight = lightLevel < minlight ? lightLevel : minlight; 
    }
  ...
  if (currentMillis - previousMillisTemp > periodTemp) {
    ...
    if (temp >= 70 && !overheated) {
      bot.sendMessage(allowed_chat_id, "Temperature is high: " + String(temp) + "\nEnter sleeping mode", "");
      bot.sendMessage(allowed_chat_id, "Sleeping... 😴", "");
      Bot_mtbs = 30000;
      sleeping = true;
      overheated = true;
      digitalWrite(LED_BUILTIN, LOW);
    } else if (temp <= 62 && overheated) {
      bot.sendMessage(allowed_chat_id, "My esp-32 brains is now cooled. I can cam again ;)", "");
      Bot_mtbs = 3000;
      sleeping = false;
      overheated = false;
    } else if (temp >= 80) {
      bot.sendMessage(allowed_chat_id, "It's too hot!", "");
      Bot_mtbs = 60000;
      sleeping = true;
      overheated = true;
      exit(0);
    ...
    while (numNewMessages) {
      ...
      for (int i = 0; i < numNewMessages; i++) {
        ...
        if (isAllowed(chat_id)) {
          ...
          if (command == "/flash") {
              if (switchLight) {
                bot.sendMessage(chat_id, "Light is already on! Use /light to turn it off", "");
                break;
              }
              flash ? bot.sendMessage(chat_id, "Now I won't flash the light before photo", "") : bot.sendMessage(chat_id, "Good, now I'm gonna flash the light before make photo!📷📸📸📸", "");
