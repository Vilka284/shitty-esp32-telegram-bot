...
bool UniversalTelegramBot::processResult(JsonObject result, int messageIndex) {
  ...
  if (last_message_received != update_id) {
    ...
    messages[messageIndex].inline_query = F("");
    ...
    if (result.containsKey("inline_query")) {
      JsonObject inline_query = result["inline_query"];
      messages[messageIndex].type = F("inline_query");
      messages[messageIndex].inline_query_id = inline_query["id"].as<String>();
      messages[messageIndex].from_id = inline_query["from"]["id"].as<String>();
      messages[messageIndex].from_name = inline_query["from"]["first_name"].as<String>();
      messages[messageIndex].date = inline_query["date"].as<String>();
      messages[messageIndex].chat_id = inline_query["chat"]["id"].as<String>();
      messages[messageIndex].chat_title = inline_query["chat"]["title"].as<String>();

      if (inline_query.containsKey("query")) {
        messages[messageIndex].text = inline_query["query"].as<String>();

      } else if (inline_query.containsKey("location")) {
        messages[messageIndex].longitude = inline_query["location"]["longitude"].as<float>();
        messages[messageIndex].latitude  = inline_query["location"]["latitude"].as<float>();
      }
      ...
      messages[messageIndex].sticker_id = message["sticker"]["file_id"].as<String>();
      messages[messageIndex].sticker_uid = message["sticker"]["file_unique_id"].as<String>();
      messages[messageIndex].sticker_width = message["sticker"]["width"].as<int>();
      messages[messageIndex].sticker_height = message["sticker"]["height"].as<int>();
      messages[messageIndex].sticker_animated = message["sticker"]["is_animated"].as<bool>();
